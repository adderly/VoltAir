...
unsigned int Game::PLAYER_CATEGORY_BITS = 0x0001;
unsigned int Game::ENEMY_CATEGORY_BITS = 0x0001;

Game::Game(QObject* parent)
    : QObject(parent),
      mPlayerProfile(this) {
    //Future subclassing will required a base instance for the engine.
    sInstance = this;
}
...
void Game::init() {
    ...
    Game::PLAYER_CATEGORY_BITS = 0x0002;
    Game::ENEMY_CATEGORY_BITS = 0x0004;
    ...
    QQmlComponent uiComponent(qmlEngine, Util::getUrlPathToAsset(m_ui_path));
...
//#if !defined(Q_OS_ANDROID)
    mUi->setFiltersChildMouseEvents(true);
//#endif
    ...
    connect(InputManager::getInstance(), &InputManager::playerActivationChanged, mUi,
    ...
    mPlayerProfile.mCloudDataEnabled = engine->cloudDataEnabled();
...
void Game::setInputManager(InputManager* iManager){
    mInputManager = iManager;
...
void Game::onLevelCompleted() {
    ...
    if (levelProgression->isMultiplayer()) {
        int numPlayers = InputManager::getInstance()->getAvailablePlayers().size();
        if (numPlayers > 1) {
...
void Game::newLevel(QString levelName){
    LevelInfo* levelInfo = new LevelInfo;
    levelInfo->setFileName("qml/editor/CustomLevel.qml");
//    levelInfo->setFileName("qml/editor/LevelCameraTest.qml");
...
void Game::onQuitRequested() {
    ...
    Engine::getInstance()->getQmlEngine()->quit();
    qApp->quit();
}
