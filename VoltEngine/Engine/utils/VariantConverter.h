...
/**
 * @ingroup Engine
 * @brief Helper template class to convert QVariant%s to and from QObject%s.
 * @see VariantConverter
 */
template <>
class VariantConverter<QObject*> : public VariantConverterBase<QObject*> {
public:
    /**
     * @brief Returns @p value converted to a QString.
     * @param value QVariant to convert
     */
    QObject* fromVariant(const QVariant& value) {
