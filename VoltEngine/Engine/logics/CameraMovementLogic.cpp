...
void CameraMovementLogic::setViewportChanged(bool value){
    mViewportChanged = value;
}
...
void CameraMovementLogic::init() {
    ...
    QObject::connect(mCamera,SIGNAL(widthChanged()),this,SLOT(screenWidthChanged()));
    QObject::connect(mCamera,SIGNAL(widthChanged()),this,SLOT(screenHeightChanged()));
...
void CameraMovementLogic::mouseWheel(QPoint pixelDelta, QPoint angleDelta)
{
    QPoint numSteps;
    if(!pixelDelta.isNull()){
        numSteps = pixelDelta;
    }else if(!angleDelta.isNull()){
        numSteps = angleDelta / 15;
    }
    zoomCamera(numSteps);
}

//TODO: use the move ratio to evaluate the amount of displacement needed.
void CameraMovementLogic::mouseMoved(QPoint pos, QPoint delta)
{
    Q_UNUSED(pos);
//    qDebug() << "move Delta  = " << delta;

    Level* level = Engine::getInstance()->getLevel();
    QRectF cameraBoundary = level->getCameraBoundary();

    qreal addToX = delta.x();
    qreal addToY = delta.y();
    qreal panDistance = .01;
    addToX = (delta.x() != 0) ? ((delta.x() > 0) ? panDistance : -panDistance) :0;
    addToY = (delta.y() != 0) ? ((delta.y() > 0) ? panDistance : -panDistance) :0;

    addToX = panDistance * delta.x();
    addToY = panDistance * delta.y();
//    qDebug() << " addtoX = " << addToX << " addtoY = " << addToY;

    cameraBoundary.adjust(addToX,addToY,addToX,addToY);
    level->setCameraBoundary(cameraBoundary);
...
void CameraMovementLogic::screenWidthChanged()
{
    mViewportChanged = true;
}

void CameraMovementLogic::screenHeightChanged()
{
    mViewportChanged = true;
}

void CameraMovementLogic::zoomCamera(QPoint zoomLevel) const
{
    Level* level = Engine::getInstance()->getLevel();
    QRectF cameraBoundary = level->getCameraBoundary();
    int wheelVal = zoomLevel.y();

    if(wheelVal>0){
        cameraBoundary.adjust(10,10,-10,-10);
    }else{
        cameraBoundary.adjust(-10,-10,10,10);
    }
    level->setCameraBoundary(cameraBoundary);
...
void CameraMovementLogic::resetCameraProperties()
{
    Level* level = Engine::getInstance()->getLevel();
    if(!level){
        qDebug() << "No Level on cam logic.";
        return;
    }

    if(level->mouseToInputArea() && level->shouldFillCamera()){
        InputArea* iArea = Engine::getInstance()->getInput();
        QObject::connect(iArea,SIGNAL(mouseWheelChanged(QPoint,QPoint)),this,SLOT(mouseWheel(QPoint,QPoint)));
        QObject::connect(iArea,SIGNAL(mouseMovedChanged(QPoint,QPoint)),this,SLOT(mouseMoved(QPoint,QPoint)),Qt::DirectConnection);
    }
