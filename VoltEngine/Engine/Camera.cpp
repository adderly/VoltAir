...
void Camera::updateTransform() {
    ...
    float p_width = p->width();
    float p_height = p->height();
    ...
    float sc = p_width / mFov;
    mTransformScale = sc;
    mTransformTranslate.setX(-(mLookAt.x() * sc) + (0.5f * p_width));
    mTransformTranslate.setY(-(mLookAt.y() * sc) + (0.5f * p_height));
    ...
    mViewportWorldBounds = QRectF(toWorldSpace(QPointF(0.0f, 0.0f)),
            toWorldSpace(QPointF(p_width, p_height)));
