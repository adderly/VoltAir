...
Body::Body(QObject* parent) : QObject(parent),mCreatedFromB2Body(false),mManualInitilization(false) {
    if(!mManualInitilization){
        initBody();
    }
}

Body::Body(b2Body *body, bool manualInit, QObject *parent):QObject(parent),
    mManualInitilization(manualInit),mBody(body)
{
    if(mBody){
...
b2Body* Body::initBody(b2BodyDef bodyDef){
    mBody = Engine::getInstance()->getWorld()->CreateBody(&bodyDef);
    connect(this, &Body::contactBegun, this, &Body::queuedContactBegun, Qt::QueuedConnection);
    connect(this, &Body::contactEnded, this, &Body::queuedContactEnded, Qt::QueuedConnection);
    return mBody;
...
void Body::setInitialX(float x){
    mInitialX = x;
    emit initialXChanged();
}

void Body::setInitialY(float y){
    mInitialY = y;
...
void Body::setCategoryBits(unsigned short bits){
    mCategoryBits = bits;
}

void Body::setGroupIndex(int group){
    mGroupIndex = group;
}
...
b2BodyDef Body::getBodyDef() {
    ...
    if (!mBody) {
        ...
        bodyDef.bullet = getIsBullet();
    }
        bodyDef.position.x = mInitialX;
        bodyDef.position.y = mInitialY;

#ifdef D_PHYSICS
    qDebug() << " initialY = " << mInitialY;
    qDebug() << " initialX = " << mInitialX;
#endif
...
bool Body::isCollidingWithBody(Body* other){
    QList<Body*>::iterator it = mContactCountMap.keys().begin();
    for(;it != mContactCountMap.keys().end();it++){
        if(*it == other){
            return true;
        }
    }
...
void Body::applyLinearImpulse(const QPointF& impulse,const QPointF& point,bool wake){
    if(!mBody){
        return;
    }
    mBody->ApplyLinearImpulse(Box2dUtil::toB2Vec2(impulse),Box2dUtil::toB2Vec2(point),wake);
}
