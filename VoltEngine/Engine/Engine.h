 ...
 *
 * TODO:
 * bugs: When resizing the window of the game, sometimes the renderers stop working or
 * matrices get fucked up.
 *
 */
...
public:
    ...
    /**
     * @brief Set the default qml loaded by the engine.
     */
    void setDefaultEngineQml(QString path) { m_default_engine_qml = path; }

    /**
     * @brief Returns the default qml file for the engine.
     */
    QString getDefaultEngineQml() const {  return m_default_engine_qml; }

    /**
     * @brief Set if the default root item will be found by name or not.
     */
    void setFindRootByName(bool byName) {m_find_root_by_name = byName; }

    /**
     * @brief Returns if the root item is found by name.
     */
    bool getFindRootByName() { return m_find_root_by_name;}

    /**
     * @brief Set if the engine will load a settings file for configuration.
     * Note: By default it gets /assets/config/engine.config
     */
    void setUseSettings(bool value) { mUseSettings = value; }

    /**
     * @brief Returns the #mUseSettings variable.
     */
    ...
    */
    ...
    /**
     * @brief Returns the InputArea object.
     */
    InputArea* getInput() const { return mInputArea; }
     ...
     */
    EngineUtils::ContactListener* getContactListener() const { return mContactListener.get(); }
    ...
    /**
    *   @brief Return the levelLoader element.
    */
    QQuickItem* getLevelLoader(){return mLevelLoader;}
    ...
    /**
    * @brief When set, the Engine will exit when ready.
    * Note: Usefull when Engine is in update(), and the level is deleted by the game.
    */
    void setRequestedExit(bool value) { mRequestedExit = value; }

    /**
    * @brief loads a simple settings file for configuring.
    */
...
public slots:
    ...
    */
    ...
    /**
    *   @brief slot for handling window width resize.
    */
    void onWidthChanged(int width);

    /**
    *   @brief slot for handling window height resize.
    */
...
private:
    ...
    QString m_default_engine_qml;
    //used to controll debugging information.
    bool mDebug = false;
    //use a config file.
    bool mUseSettings = false;
    bool m_find_root_by_name = false;
    ...
    std::unique_ptr<EngineUtils::ContactListener> mContactListener;
