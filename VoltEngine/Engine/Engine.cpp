...
Engine::Engine(QObject* parent) : QObject(parent) {
    m_default_engine_qml = "Engine/qml/main.qml";
}
...
void Engine::init() {
    ...
    mViewer->setSource(Util::getUrlPathToAsset(m_default_engine_qml));
    ...
    connect(mViewer.get(),SIGNAL(widthChanged(int)),this,SLOT(onWidthChanged(int)));
    connect(mViewer.get(),SIGNAL(heightChanged(int)),this,SLOT(onHeightChanged(int)));
    ...
    mSoundManager.reset(new SoundManager(this,mSoundsMuted));
    ...
    mContactListener.reset(new EngineUtils::ContactListener());
...
void Engine::performLevelChange() {
    ...
    // has been cleared.
    if (mLevelLoading  || !mHasNextLevel || mSyncsSinceNextLevelRequest <= 0) {
        return;
    }
    mLevelLoading = true;
    mHasNextLevel = false;
    ...
    if(mDefaultLevelPath){
        mLevelLoader->setProperty("source", Util::getPathToLevel(mNextLevelName));
    }else{
        mLevelLoader->setProperty("source", Util::getPathToAsset(mNextLevelName));
    }

    qDebug() <<  "Level load path : " << mLevelLoader->property("source");
...
void Engine::onLoadCompleted(bool success) {
    ...
    qDebug() <<  (success ?  "Level load : File loaded." :  "Level load : File loaded.");
    if (success) {
        ...
        QString levelPath = QString::fromStdString(mInFlightLevelInfo->getFileName().toStdString());

       // std::this_thread::sleep_for(std::chrono::seconds(1));
        QFileInfo levelFileInfo(Util::getPathToLevel(levelPath));
        ...
        //TODO: randoms sigfaults loading the sound here.
        //Good idea would be to load them asynchronously
        if(mInFlightLevelInfo->hasEnvironment()){
            QString bgm = mInFlightLevelInfo->getEnvironment()->getBGMTrack();
            if(!bgm.isEmpty()){
                mSoundManager->setBGMTrack(SoundManager::EnvironmentalPriority,
                        Util::getPathToSound(bgm));
            }
        }
...
void Engine::debugRender() {
    ...
    if (!mDebugHudItem && mViewer) {
        mDebugHudItem = getRoot()->findChild<QQuickItem*>("debugHud");
    }
...
void Engine::showViewer() {
    if (mViewer && !mViewer->isVisible()) {
        if(mFullscreen){
            ...
            mViewer->resize(defaultW,defaultH);
            mViewer->show();
        }
    }
}
...
void Engine::update() {
    ...
    forEach<Logic>([] (Logic* logic) {
        if (logic->isActive() && !logic->isManaged()) {
            logic->update();
        }
    });
...
void Engine::onQuitRequested() {
    ...
    mSoundManager->clearBGMTrack(SoundManager::MaxPriority);
}
...
void Engine::onWidthChanged(int width)
{
    Q_UNUSED(width)
}

void Engine::onHeightChanged(int height)
{
    Q_UNUSED(height)
}
